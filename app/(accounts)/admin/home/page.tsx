...
export default function Homepage() {
  ...
  const { data, error, isLoading } = useSWR(performFilter, getUser);
        ...
        <div className=" fixed bottom-24 right-24 bg-white h-96 rounded-t-xl  shadow-lg w-80 z-50 animate__animated animate__bounceIn">
          <div className="flex flex-row justify-between bg-darkviolet p-3 rounded-t-xl text-white">
            <div className=" text-sm  flex gap-2 items-center">
              ...
              <span className="font-semibold">LazBot</span>
            </div>
          ...
          <div className="absolute bottom-0 w-full flex flex-row  border p-2">
            <input
              type="text"
              placeholder="Say something..."
              className="px-2 text-sm focus:outline-none h-9 w-full"
            />
            <button className="text-white bg-darkviolet rounded-lg p-2">
      ...
      <div className="h-[70vh] min-h-[70vh] flex flex-col justify-center bg-[url(https://images.pexels.com/photos/4904563/pexels-photo-4904563.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1)] bg-center bg-cover bg-brandviolet/20  bg-blend-soft-light">
        <div className="p-10 flex flex-col gap-2">
          <div className="flex flex-col text-white font-semibold text-4xl md:text-5xl">
            <div>Discover the joy of</div>
            <div>online shopping</div>
          </div>

          <div>
            <button className="px-2 w-36 p-2 border text-white border-white text-xs hover:bg-white/20 ">
              Shop now
            </button>
          </div>
        </div>
      ...
      <div className="w-full my-10  flex flex-col gap-5 justify-center items-center ">
        <div className="w-full flex flex-col gap-2  max-w-3xl md:max-w-4xl lg:max-w-7xl px-5 h-full">
          <div className="flex flex-row justify-between items-center">
            <div className="font-semibold capitalize text-customblack">
              {" "}
              Top{" "}
            ...
            <button
              ...
              className="p-2 bg-darkviolet text-white rounded-lg focus:outline-none  w-20 flex gap-1 text-xs justify-center items-center hover:bg-darkviolet/80"
            >
            ...
            <div className="  grid grid-cols-2 md:grid-cols-3 gap-5">
              {products.map((data: any) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="bg-white cursor-pointer p-5 flex flex-col  items-center shadow-lg rounded-xl relative gap-5"
                      key={id}
                    >
                      <button className="absolute bg-red-300 shadow-xl right-0 p-2 top-5 flex flex-row items-center justify-center gap-1 text-white">
                        <BsHeartFill className="h-5 w-5  hover:text-darkviolet" />
                      ...
                      <div className="flex flex-col gap-3 w-full">
                        <div className="text-customblack font-semibold text-sm truncate max-w-sm">
                          {title}
                        </div>
                        <div className="flex justify-between">
                          <div className="flex flex-col">
                            <div className="capitalize text-xs bg-brandgray rounded-full px-2 py-1 text-white">
                              {category}
                            </div>
                            <div className="text-gray-500">${price}</div>
                          </div>
                          <button className=" bg-darkviolet text-white p-2 rounded-full w-10 h-10  text-sm flex items-center justify-center">
          ...
          <div className="flex flex-col gap-5  pt-10">
            <div className="w-full text-center font-semibold text-4xl  text-customblack p-5 ">
              Why Choose Us?
            </div>
            {choose_us?.map(({ id, title, image, desc }) => (
              ...
              >
                <div
                  className={`flex ${
                    id % 2 === 0
                      ? "flex-col md:flex-row-reverse divide-x-0 md:divide-x-2 md:divide-x-reverse"
                      : "flex-col md:flex-row divide-x-0 md:divide-x-2"
                  } justify-evenly w-full  bg-white rounded-lg shadow-lg p-5`}
                  key={id}
                >
      ...
      <div className="bg-white animate__animated animate__zoomIn w-full max-w-md p-5 rounded-lg">
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-row justify-evenly gap-3">
            <button
              className="w-full bg-darkviolet text-white  p-2 text-sm hover:bg-darkviolet/80"
              onClick={() =>
            ...
            <button
              className="w-full border border-red-400 text-red-400 hover:bg-red-400/20  p-2 text-sm"
